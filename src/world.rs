...
pub fn world((ns, ss): Seeds, vp @ (x, y, w, h): Viewport) -> Vec<Vec<u64>> {
    if y >= 0 {
        ...
        let mut north_xs = hemisphere(ns, Direction::Reverse, vp_n);
...
impl Longitudes {
    pub fn take_finite((e_seed, w_seed): Seeds, x: i64, w: usize) -> Vec<u64> {
        ...
        if x >= 0 {
            ...
            // bordering
            let west_xs = Longitudes::take_from_one_dir(
                Direction::Normal,
                w_seed,
                0,
                (w as i64 + x) as usize,
            ...
            let mut east_xs =
                Longitudes::take_from_one_dir(Direction::Reverse, e_seed, skips, skips);
            east_xs.extend(west_xs);
            east_xs
        }
    }

    fn take_from_one_dir(dir: Direction, seed: u64, skips: usize, w: usize) -> Vec<u64> {
